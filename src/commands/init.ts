...
export async function initCommand(): Promise<void> {
  ...
  logger.dim('  1. pbs add <git-url>        # Add a playbook source');
  logger.dim('     pbs add --local <path>   # Or add a local source');
  logger.dim('  2. pbs sync                 # Sync to .opencode/skills (opencode enabled by default)');
  logger.dim('     # To enable more targets: edit playbook-sync.yaml → cursor / copilot / claude');
}
